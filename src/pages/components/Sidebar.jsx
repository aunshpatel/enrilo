...
// import { ChevronLeft, ChevronRight, FileText, Users, Settings } from "lucide-react";
...
//   const menuItems = [
//     { name: "Consultancy Form", path: "/consultancy", icon: <FileText /> },
//     { name: "All Consultancies", path: "/", icon: <Users /> },
//     { name: "Payment Details", path: "#", icon: <FileText /> },
//     { name: "Super Admin", path: "#", icon: <Users /> },
//     { name: "Company Details", path: "#", icon: <Settings /> },
//   ];

//   return (
//     <aside className={`h-screen bg-[#1E293B] text-white flex flex-col justify-between transition-all duration-300 ${ isOpen ? "w-64" : "w-20" }`}>
//       <div className="p-4">
...
//             <li key={idx}>
//               <Link to={item.path} className={`flex items-center space-x-3 p-2 rounded-md hover:bg-[#334155] transition ${ location.pathname === item.path ? "bg-[#334155]" : "" }`} >
//                 <span>{item.icon}</span>
...
export default function Sidebar() {
  ...
  const [activeMenu, setActiveMenu] = useState(null); // controls open/close of submenu
  const location = useLocation();
  ...
  useEffect(() => {
    ...
    else localStorage.removeItem("activeMenu");
  }, [activeMenu]);

  // --- Automatically open submenu if location matches ---
  useEffect(() => {
    const matchedMenu = menuItems.find(
      (item) => item.subMenu?.some((sub) => sub.path === location.pathname) || item.path === location.pathname
    );
    if (matchedMenu?.subMenu) {
      setActiveMenu(matchedMenu.name); // open submenu automatically
    } else {
      setActiveMenu(null); // close all if no submenu
    }
  }, [location.pathname]);
  ...
  const renderMenuItems = () =>
    menuItems.map((item, idx) => {
      ...
      const isMenuActive = item.path === location.pathname;
            ...
            <>
              <button
                ...
                className={`flex items-center justify-between w-full p-2 rounded-md transition-all duration-200
                  ${isSubActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]"}
                `}
              >
                ...
                {isOpen && <span>{activeMenu === item.name ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</span>}
              </button>

              {hasSubMenu && isOpen && activeMenu === item.name && (
                      ...
                      <li key={subIdx}>
                        <Link
                          ...
                          className={`flex justify-between items-center text-sm rounded-md px-3 py-2 transition
                            ${isActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#475569]"}
                          `}
                        >
                          <span>{sub.icon}</span>
                          <span>{sub.name}</span>
...
//   // --- Open submenu when path matches ---
//   useEffect(() => {
//     const matchedMenu = menuItems.find(
//       (item) => item.subMenu?.some((sub) => sub.path === location.pathname) || item.path === location.pathname
//     );
//     if (matchedMenu) setActiveMenu(matchedMenu.name);
//   }, [location.pathname]);
...
//     menuItems.map((item, idx) => {
//       const hasSubMenu = !!item.subMenu;
//       const isMenuActive = activeMenu === item.name || location.pathname === item.path;
//       const isSubActive = item.subMenu?.some((sub) => sub.path === location.pathname);

//       return (
//         <li key={idx}>
//           {hasSubMenu ? (
//             <button onClick={() => toggleSubMenu(item.name)} className={`flex items-center justify-between w-full p-2 rounded-md transition-all duration-200 ${ isSubActive || isMenuActive ? "bg-[#334155]" : "hover:bg-[#334155]" }`} >
//               <div className="flex items-center space-x-3">
//                 <span>{item.icon}</span>
//                 {isOpen && <span className="text-sm font-medium">{item.name}</span>}
//               </div>
//               {isOpen && (
//                 <span>{activeMenu === item.name ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</span>
//               )}
//             </button>
//           ) : (
...
//           {hasSubMenu && isOpen && isMenuActive && (
//             <ul className="ml-10 mt-2 space-y-1">
//               {item.subMenu.map((sub, subIdx) => {
//                 const isActive = location.pathname === sub.path;
//                 return (
...
//                     {/* <Link to={sub.path} className={`block text-sm rounded-md px-3 py-2 transition ${ isActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#475569]" }`}> */}
//                     <Link to={sub.path} className={`flex justify-between text-sm rounded-md px-3 py-2 transition ${ isActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#475569]" }`}>
//                       <span>{sub.icon}</span>
//                       {sub.name}
//                     </Link>
//                   </li>
...
//       {/* --- Mobile Overlay --- */}
//       {mobileOpen && <div className="fixed inset-0 bg-black/50 z-30 md:hidden" onClick={() => setMobileOpen(false)}></div>}
//     </>
//   );
// }